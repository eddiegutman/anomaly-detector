...
void SimpleAnomalyDetector::learnNormal(const TimeSeries &ts) {
    for (auto firstIt = ts.Data.begin(); std::next(firstIt, 1) != ts.Data.end(); firstIt++) {
        ...
        for (auto secondIt = std::next(firstIt, 1); secondIt != ts.Data.end(); secondIt++) {
            float p = pearson((float*)firstIt->second.data(), (float*)secondIt->second.data(), firstIt->second.size());
            p = std::abs(p);
        ...
        if (maxPearson != 0) {
            int size = ts.Data.begin()->second.size();
            Point* points[size];
            for (int i = 0; i < size; i++) {
                points[i] = new Point(firstIt->second[i], ts.Data.find(secondFeature)->second[i]);
            }
...
vector<AnomalyReport> SimpleAnomalyDetector::detect(const TimeSeries &ts) {
    ...
    int lines = ts.Data.begin()->second.size();
    for (int i = 0; i < lines; i++) {
        for (auto & it : cf) {
            float x = ts.Data.find(it.feature1)->second[i];
            float y = ts.Data.find(it.feature2)->second[i];
            Point p = Point(x, y);
